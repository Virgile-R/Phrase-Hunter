...
class Game {
    constructor(){
        this.missed = 0
        this.phrases = ["I am Programming Java", 
    ...
    getRandomPhrase(previousGamePhrase){
        let nextPhrase = this.phrases[Math.floor(Math.random() * this.phrases.length)]
        while (nextPhrase === previousGamePhrase) {
            nextPhrase = this.phrases[Math.floor(Math.random() * this.phrases.length)]
        }
    ...
    handleInteraction(e){
        ...
        if (!this.phraseToString.includes(e.target.textContent)) {
           ...
           this.activePhrase.showMatchedLetter(this.activePhrase.checkLetter(e.target.textContent))
            if (this.checkForWin()) {
                this.gameOver()
            }
        }
    ...
    removeLife(){
        const scoreBoardList = document.getElementById("scoreboard").firstElementChild
        if (this.missed <=4){
            scoreBoardList.children[scoreBoardList.children.length - (1 + this.missed) ].style.display= "none"
        } else {
    ...
    checkForWin(){
        ...
        if (Array.from(phraseLetters)
            .filter(classes  => classes.className.includes("hide"))
            .length === 0){ 
                hasWon = true
                return true
            }
